...
log = _LoggerFactory.get_logger('fuse')
...
def contains_write_flag(flags: int) -> bool:
    return flags & (os.O_APPEND | os.O_WRONLY | os.O_RDWR) != 0
...
class FUSE(object):
    ...
    def open(self, path, fip):
        ...
        if self.raw_fi:
            return self.operations('open', path.decode(self.encoding), fi.flags, fi)
        else:
    ...
    def write(self, path, buf, size, offset, fip):
        ...
        return self.operations('write', self._decode_optional_path(path), size,
                                        offset, fh, buf)
    ...
    def create(self, path, mode, fip):
        ...
        if self.raw_fi:
            return self.operations('create', path, mode, fi.flags, fi)
        else:
...
class Operations(object):
    ...
    def write(self, path, size, offset, fh, buf):
        raise FuseOSError(errno.EROFS)
